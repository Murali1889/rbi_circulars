...
const Sidebar = () => {
    ...
    const menuItems = [
        ...
        { name: 'MeitY', path: '/meity', color: '#1A3C34', hover: '#122C26', light: '#D1E8E2', accent: '#2D6A5D' },
    ];
    return (
        <div
            ...
            <nav className="mt-4">
                {menuItems.map((item) => (
                    <Link
                        key={item.path}
                        to={item.path}
                        className={`flex items-center px-6 py-3 transition-all duration-200 hover:bg-[${item.light}] ${location.pathname.startsWith(item.path)
